...
class _BedSidebarState extends State<BedSidebar> {
  ...
  Widget _buildNode(Map<String, dynamic> node, {int depth = 0}) {
    ...
    final isLeaf = isRoom || (node['children'] == null && !['ROOT','CAMPUS','PARK','BUILDING','UNIT','FLOOR'].contains(nodeType));
    if (isLeaf) {
    ...
    IconData icon;
    switch (nodeType) {
      case 'ROOT': icon = Icons.school_rounded; break;
      case 'CAMPUS': icon = Icons.location_city_rounded; break;
      case 'PARK': case 'BUILDING': icon = Icons.apartment_rounded; break;
      case 'UNIT': icon = Icons.view_day_rounded; break;
      case 'FLOOR': icon = Icons.layers_rounded; break;
      default: icon = Icons.folder_rounded;
    }
    return ExpansionTile(
      tilePadding: const EdgeInsets.symmetric(horizontal: 8),
      childrenPadding: EdgeInsets.only(left: 8.0 + depth * 8),
      leading: Icon(icon, size: 16, color: depth < 2 ? primaryColor : textSecondary),
      title: Text(name,
          style: TextStyle(fontSize: 12 + (depth < 2 ? 1 : 0),
              fontWeight: depth < 2 ? FontWeight.w600 : FontWeight.w500,
              color: depth < 3 ? textPrimary : textSecondary)),
      initiallyExpanded: depth < 3,
      children: children.map((c) => _buildNode(c as Map<String, dynamic>, depth: depth + 1)).toList(),
    );
  }
}
